...
double Geometry::estimate_curvature_normal_variation(
	const double* n0_in,
	const double* neighbor_normals,
	size_t num_neighbors,
	size_t stride)
{
	// This is a curvature proxy (not geometric curvature in units of 1/length):
	// we measure how quickly normals change around a point.
	if (!n0_in || !neighbor_normals || num_neighbors == 0) return DBL_MAX;
	if (stride < 3) return DBL_MAX;

	double n0[3] = { n0_in[0], n0_in[1], n0_in[2] };
	if (!normalize_vector(3, n0)) return DBL_MAX;

	double sum_angle = 0.0;
	size_t used = 0;
	for (size_t i = 0; i < num_neighbors; i++) {
		const double* ni_in = neighbor_normals + i * stride;
		double ni[3] = { ni_in[0], ni_in[1], ni_in[2] };
		if (!normalize_vector(3, ni)) continue;

		double d = dot_product(3, n0, ni);
		if (d > 1.0) d = 1.0;
		if (d < -1.0) d = -1.0;
		sum_angle += acos(d);
		used++;
	...
	if (used == 0) return DBL_MAX;
	return sum_angle / (double)used;
}
