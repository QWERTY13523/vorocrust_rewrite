...
int main()
{
    FixMeshNormals("../data/obj/Ours_8000_block_Remesh_constraint_3_modified.obj", "../data/obj/Ours_8000_block_Remesh_constraint_3_modified_Fixed.obj");
    Generator generator;
    ...
    // generator.read_input_obj_file("../data/obj/bunny.obj",num_points,points,num_faces,faces);
    // if (num_points == 0) {
    //     std::cerr << "[Error] Failed to read input points or empty file." << std::endl;
    //     return 1;
    // }

    generator.generate_spheres("../data/spheres/Sphere_8000_65_block_constraint_3.csv", spheres);
    if (spheres->get_num_tree_points() == 0) {
        std::cerr << "[Error] Failed to load spheres or empty file." << std::endl;
        return 1;
    }

    generator.read_obj_faces("../data/obj/Ours_8000_block_Remesh_constraint_3_modified_Fixed.obj",face_flat, num_faces1);
    if (face_flat.empty()) {
    ...
    generator.generate_surface_seeds(spheres, upper_seeds, lower_seeds, num_faces1, face_flat);
    ...
    generator.generate_seed_csv_with_faces("seeds.csv", seeds, spheres, "seed_faces.obj");
    generator.generate_surface_mesh(seeds, spheres, "surface_mesh1.obj");
    //Inout::analyze_seed_csv("seeds.csv");

    // Offset::PowerDiagramParams params;
    // params.r_value = 0.1; // R, gradient=0 (<1). Change as needed.
    // params.r_signed_by_region = true;
    // params.vertex_merge_epsilon = 1e-2;
    // std::string err;
    // if (!Offset::build_power_diagram_from_seed_csv(
    //         "seeds.csv",
    //         "power_outside.obj",
    //         "power_inside.obj",
    //         "power_surface.obj",  // interface between inside/outside pairs
    //         "power_basepoints.obj",
    //         "power_offsets.obj",
    //         params,
    //         &err)) {
    //     std::cerr << "Power diagram failed: " << err << std::endl;
    // }
}
