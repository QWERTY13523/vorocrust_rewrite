...
// --- 前置声明 ---
std::vector<Point> getVoronoiFace(const Delaunay& dt, Vertex_handle v1, Vertex_handle v2);
...
void optimize_seeds_remove_slivers(MeshingTree* seeds) {
    ...
    double perturb_step = bbox_diag * 1e-4;  // 扰动步长 (非常小，约 1/100000)
    int max_iters = 8; // 最大迭代次数
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        // --- 构建临时 DT ---
        Delaunay dt;
        std::vector<Vertex_handle> vhs(num_seeds); // 映射: seed_idx -> vertex_handle
        
        for (size_t i = 0; i < num_seeds; i++) {
            if (!seeds->tree_point_is_active(i)) continue;
            double* pt = seeds->get_tree_point(i);
            Vertex_handle vh = dt.insert(Point_3(pt[0], pt[1], pt[2]));
            vh->info() = i; // 存储 seed index
            vhs[i] = vh;
        }
        ...
        for (auto cit = dt.finite_cells_begin(); cit != dt.finite_cells_end(); ++cit) {
            // 计算 Voronoi 顶点 (四面体外心)
            Point_3 dual_pt = dt.dual(cit);
            
            // 检查是否飞向无穷远
            double x = CGAL::to_double(dual_pt.x());
            double y = CGAL::to_double(dual_pt.y());
            double z = CGAL::to_double(dual_pt.z());
            
            if (std::abs(x) > far_threshold || std::abs(y) > far_threshold || std::abs(z) > far_threshold) {
                bad_cells++;
                // 标记该 cell 的 4 个顶点进行扰动
                for (int i = 0; i < 4; ++i) {
        ...
        // --- 扰动 (Perturbation) ---
        // 直接修改 MeshingTree 中的点坐标
        for (size_t seed_idx : seeds_to_perturb) {
            double* pt = seeds->get_tree_point(seed_idx);
            
            // 施加随机微小位移
            pt[0] += dist(rng) * perturb_step;
            pt[1] += dist(rng) * perturb_step;
            pt[2] += dist(rng) * perturb_step;
        }
    }
